...
# Native SStream layout: no transpose-contiguous K/V copies.
q_bhmd = q.transpose(1, 2).contiguous()
k_bhnd = k.transpose(1, 2).contiguous()
v_bhnd = v.transpose(1, 2).contiguous()
out3, lse3, colsum3 = attn_cuda.flash_attn_bias_colsum_bhnd(
...
`flash_attn_bias_colsum_bhnd` instead accepts contiguous Q `[B,H,M,D]`
and K/V `[B,H,N,D]`, returning output `[B,H,M,D]`. This path is intended
for SStreamVGGT's native cache layout and avoids full-cache layout copies.
