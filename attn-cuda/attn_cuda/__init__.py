...
from attn_cuda._ext import is_available, get_version, flash_attn_fwd, flash_attn_fwd_bhnd
...
def flash_attn_bias_colsum_bhnd(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    bias: Optional[torch.Tensor] = None,
    ...
    subsample_ratio: float = 1.0,
           ...
           Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]:
    """Flash attention for contiguous SStream tensors.

    Q is [B, H, M, D], K/V are [B, H, N, D], and output is [B, H, M, D].
    LSE and column-sum retain the shared [B, H, M/N] layouts.
    """
    if softmax_scale is None:
        softmax_scale = q.shape[-1] ** (-0.5)
    results = flash_attn_fwd_bhnd(
        q, k, v, softmax_scale, bias, return_colsum, subsample_ratio)
    if return_colsum:
        return results[0], results[1], results[2]
    return results[0], results[1]
