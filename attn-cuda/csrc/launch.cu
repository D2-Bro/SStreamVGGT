    ...
    bool head_major)
{
    ...
    params.seqlen_q = q.size(head_major ? 2 : 1);
    params.seqlen_k = k.size(head_major ? 2 : 1);
    params.num_heads = q.size(head_major ? 1 : 2);
    params.head_dim = q.size(3);

    int const row_dim = head_major ? 2 : 1;
    int const head_dim = head_major ? 1 : 2;
    params.q_row_stride = q.stride(row_dim);
    params.k_row_stride = k.stride(row_dim);
    params.v_row_stride = v.stride(row_dim);
    params.o_row_stride = out.stride(row_dim);
    params.q_head_stride = q.stride(head_dim);
    params.k_head_stride = k.stride(head_dim);
    params.v_head_stride = v.stride(head_dim);
    params.o_head_stride = out.stride(head_dim);
