...
#define ATTN_CUDA_VERSION_PATCH 1
...
std::vector<torch::Tensor> flash_attn_fwd_impl(
    torch::Tensor q, torch::Tensor k, torch::Tensor v,
    float softmax_scale,
    c10::optional<torch::Tensor> bias,
    bool return_colsum,
    ...
    bool head_major)
{
    ...
    TORCH_CHECK(q.dim() == 4 && k.dim() == 4 && v.dim() == 4,
                "q, k, and v must be 4D");

    const int B = q.size(0);
    const int M = q.size(head_major ? 2 : 1);
    const int H = q.size(head_major ? 1 : 2);
    const int D = q.size(3);
    const int N = k.size(head_major ? 2 : 1);

    TORCH_CHECK(D == 64, "Only head_dim=64 is supported, got ", D);
    TORCH_CHECK(k.size(0) == B && v.size(0) == B, "Batch size mismatch");
    TORCH_CHECK(v.size(head_major ? 2 : 1) == N, "K/V sequence length mismatch");
    TORCH_CHECK(k.size(head_major ? 1 : 2) == H && v.size(head_major ? 1 : 2) == H,
                "Head count mismatch");
    ...
    auto opts = q.options();
    torch::Tensor out = head_major
        ? torch::empty({B, H, M, D}, opts)
        : torch::empty({B, M, H, D}, opts);
...
std::vector<torch::Tensor> flash_attn_fwd(
    ...
    float subsample_ratio)
{
    return flash_attn_fwd_impl(
        q, k, v, softmax_scale, bias, return_colsum, subsample_ratio, false);
}

std::vector<torch::Tensor> flash_attn_fwd_bhnd(
    torch::Tensor q, torch::Tensor k, torch::Tensor v,
    float softmax_scale,
    c10::optional<torch::Tensor> bias,
    bool return_colsum,
...
{
    return flash_attn_fwd_impl(
        q, k, v, softmax_scale, bias, return_colsum, subsample_ratio, true);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("flash_attn_fwd_bhnd", &flash_attn_fwd_bhnd,
          "Flash attention forward for contiguous [B, H, M/N, D] tensors",
          py::arg("q"), py::arg("k"), py::arg("v"),
          py::arg("softmax_scale"),
          py::arg("bias") = py::none(),
          py::arg("return_colsum") = false,
