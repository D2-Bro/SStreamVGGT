...
layer_budget_strategy='covariance_pr' #[leverage_pr, uniform, leverage_entropy, value_weighted_leverage_pr]
...
output_dir="${workdir}/eval_results/mv_recon/S${model_name}_${max_frames}_layerBudget_${layer_budget_strategy}_a${layer_budget_alpha}_${leverage_eviction_selector}_block${leverage_dpp_greedy_block_size}_qb${leverage_dpp_quality_beta}_db${leverage_dpp_diversity_beta}_dppProjRidge_r${leverage_ridge_dim}_rightSketchRidge_r${leverage_ridge_dim}_min${layer_budget_min_tokens}_budget${total_budget}_histCamMove_recency_robust"
echo "$output_dir"

accelerate launch --num_processes 4 --main_process_port 29902 ./eval/mv_recon/launch.py \
