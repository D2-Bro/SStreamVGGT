...
for data in "${datasets[@]}"; do
    output_dir="${workdir}/eval_results/video_depth/${data}_S${model_name}_layerBudget_${layer_budget_strategy}_a${layer_budget_alpha}_min${layer_budget_min_tokens}_${leverage_eviction_selector}_block${leverage_dpp_greedy_block_size}${leverage_approx_tag}_budget${total_budget}_kf${kf_interval}_dppRatio${leverage_dpp_diversity_beta}_norm"
    echo "$output_dir"
